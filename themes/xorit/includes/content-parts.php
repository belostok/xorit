 ...
 */
function xorit_inline_style( string $name, bool $echo = true ): string {
	global $xorit_used_inline_styles;
	if ( ! is_array( $xorit_used_inline_styles ) ) {
		return '';
	}

	// Check is this stylesheet was already added on the page
	$is_added = in_array( $name, $xorit_used_inline_styles, true );
	if ( ! $is_added ) {
		$xorit_used_inline_styles[] = $name;
	}
	...
	if ( ! $is_added || wp_doing_ajax() ) {
		...
		$path = XORIT_THEME_PATH . "build/inline/$name.css";
		...
		if ( $wp_filesystem->exists( $path ) ) {
			...
			// Add unique ID
			$id = 'xorit-' . str_replace( '_', '', $name ) . '-inline-css';
			if ( $is_added ) {
				$id = uniqid( "$id-" );
			}
 ...
 */
function xorit_the_image( $image, string $classes = '', $size = 'full', $extra_attributes = '', bool $print = true ) {
	// Get image classes
 ...
 */
function xorit_the_logo() {
	if ( XORIT_IS_MOB ) {
		$custom_logo_id = get_theme_mod( 'xorit_mobile_logo' ) ? get_theme_mod( 'xorit_mobile_logo' ) : get_theme_mod( 'xorit_logo' );
	} else {
		$custom_logo_id = get_theme_mod( 'xorit_logo' );
	}
	...
	if ( $custom_logo_id ) {
		...
		<a class="custom-logo-link img-middle"
			...
			rel="home">
			<?php xorit_the_image( $custom_logo_id, 'custom-logo' ); ?>
		</a>
		<?php
	}
 ...
 * @param bool $print Optional. Print or just return the markup.
 */
function xorit_the_link( array $link, string $classes = '', string $extra_data = '', bool $print = true ) {
	$link_url    = $link['url'] ?? '#';
